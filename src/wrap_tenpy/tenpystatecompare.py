...
psi_up_z = tenpy.networks.mps.MPS.from_product_state(M.lat.mps_sites(), ['up'] * n_spins, "finite")
psi_down_z = tenpy.networks.mps.MPS.from_product_state(M.lat.mps_sites(), ['down'] * n_spins, "finite")

psi_in = psi_up_z.add(psi_down_z, 1/np.sqrt(2), 1/np.sqrt(2))

bond_dim_vals = np.logspace(0, n_spins//2, num=n_spins//2+1, base=2)

# create a two dimensional array, indexed by bond dimension and time
...
for index_t, t in enumerate(tlist):    
    ...
    for index_bond_dim, bond_dim in enumerate(bond_dim_vals):
        ...
        inner_products_bd[index_t, index_bond_dim] = \
            np.abs(psi_ref.overlap(psi))**2 / np.abs(psi_ref.overlap(psi_ref))**2

# Moments
xx_nn = np.zeros((n_steps, len(bond_dim_vals)), dtype=float)
xy_nn = np.zeros((n_steps, len(bond_dim_vals)), dtype=float)
xz_nn = np.zeros((n_steps, len(bond_dim_vals)), dtype=float)



la, lb = n_spins//2-1, n_spins//2
for index_t, (t, psi) in enumerate(zip(tlist, psi_ts)):
    ...
    for index_bond_dim, bond_dim in enumerate(bond_dim_vals):
        ...
        xx_nn[index_t, index_bond_dim] = \
            4 * psi.expectation_value_term([('Sx', la), ('Sx', lb)]) 
        
        xy_nn[index_t, index_bond_dim] = \
            4 * psi.expectation_value_term([('Sx', la), ('Sy', lb)]) 

        xz_nn[index_t, index_bond_dim] = \
            4 * psi.expectation_value_term([('Sx', la), ('Sz', lb)]) 
...
plt.savefig(prefix + '_tenpy_%dspins.pdf' % (n_spins,))
...
for index_bond_dim, bond_dim in enumerate(bond_dim_vals):
    ...
    ax.plot(tlist/2/pi, xx_nn[:, index_bond_dim], \
            label='xx @' + label)

    ax.plot(tlist/2/pi, xy_nn[:, index_bond_dim], \
            label='xy @' + label)

    ax.plot(tlist/2/pi, xz_nn[:, index_bond_dim], \
            label='xz @' + label)
...
prefix = time.strftime('%Y-%m-%d_%H-%M-%S')
plt.savefig(prefix + '_tenpy_%dspins_moments.pdf' % (n_spins,))
