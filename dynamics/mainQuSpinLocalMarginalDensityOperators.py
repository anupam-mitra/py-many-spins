...
if __name__ == '__main__':
    argument_parser = argparse.ArgumentParser(
        prog="quspin_marginalize",
        description="Calculates local marginals from a pickled QuSpin state.",
        epilog="",
    )
    ...
    systemsize = int(np.log2(np.asarray(state).shape[0]))
    if 2**systemsize != np.asarray(state).shape[0]:
        raise ValueError("QuSpin state length is not a power of two")
    basis = spinhalf_basis(systemsize)
    ...
    for sites_sel in selectsites_list:
        ...
        rho = local_marginal_density_matrix(state, sites_sel, basis)
